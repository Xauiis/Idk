...
> **Build status:** ✅ **Phases 0–7 (solo arc) are implemented and playable** — **19 skills** (grouped Gather→Refine→Conjure→Craft→Shop), the 5-element system, a ~28-recipe combination web, the deterministic quality system (Crude→Pristine) with min-quality orders, the Lore research log, all three product trees, a managed **Aether/Quintessence economy**, **Inscription** with programmable line caps, **The Lab** dashboard, a **world of 6 biomes** (Prospecting & Tidewalking), a **four-season cycle** with festivals, **reputation-tiered** order slots, **Husbandry**/**Astrology**/**Curation** with a moon-phase cycle and **Coziness**, an **Almanac** of 15 achievements, and Phase 7's **Great Work** endgame (the four-stage Philosopher's Stone) + **New Bloom** prestige (New Game+ with a stacking legacy bonus) + export/import save codes. A 90-minute headless playtest runs in CI as a balance regression.
>
> **Remaining (server-dependent, out of scope for this client-only build):** cloud save sync, shared codex, leaderboards, and live multiplayer events — these need a backend.
