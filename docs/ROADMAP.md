...
> **Build status:** ✅ **Phases 0–6** are implemented and playable — **19 skills** (grouped Gather→Refine→Conjure→Craft→Shop), the 5-element system, a ~28-recipe combination web, the deterministic quality system (Crude→Pristine) with min-quality orders, the Lore research log, all three product trees, a managed **Aether/Quintessence economy**, **Inscription** with programmable line caps, **The Lab** dashboard, a **world of 6 biomes** (Prospecting & Tidewalking), a **four-season cycle** with festivals, **reputation-tiered** order slots, plus Phase 6's **Husbandry** (creatures, incl. an Aether source), **Astrology** (stardust + a moon-phase cycle whose four phases each grant a timing boon), **Curation** decor + a **Coziness** stat, and an **Almanac** of 15 achievements. A 90-minute headless playtest runs in CI as a balance regression. **Phase 7 (endgame & prestige) is the next target.**
